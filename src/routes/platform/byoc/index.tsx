...
import { ServerIcon, CloudIcon, TerminalIcon, ShieldCheckIcon } from "lucide-qwik";
      ...
      <main class="flex-1 overflow-y-auto p-8 lg:p-12 mb-20 max-w-5xl mx-auto w-full pt-[104px]">
        ...
        <div class="grid lg:grid-cols-3 gap-6 mb-12">
          <div class="rounded-xl border border-outline-variant/10 bg-surface-container-low p-6">
            ...
            <p class="text-sm text-tertiary">Download one ~45MB file. No Docker required. Runs on macOS, Linux, Windows.</p>
          </div>
          <div class="rounded-xl border border-outline-variant/10 bg-surface-container-low p-6">
            <CloudIcon class="w-8 h-8 text-primary mb-4" />
            <h3 class="font-bold text-lg mb-2">Any Cloud</h3>
            <p class="text-sm text-tertiary">AWS EC2, GCP Compute Engine, Azure VM, or your own data center.</p>
          </div>
          <div class="rounded-xl border border-outline-variant/10 bg-surface-container-low p-6">
            <ShieldCheckIcon class="w-8 h-8 text-primary mb-4" />
            <h3 class="font-bold text-lg mb-2">Air-Gapped Ready</h3>
            <p class="text-sm text-tertiary">Embedded embeddings via Candle/ONNX. No external API calls needed. Works offline.</p>
          </div>
        </div>
        ...
        <div class="rounded-xl border border-outline-variant/10 bg-surface-container-low p-8 mb-8">
          ...
          <div class="space-y-6">
            <div>
              ...
              <pre class="bg-black/40 rounded-lg p-4 text-sm text-tertiary overflow-x-auto">curl -L https://github.com/sharjeel619/aletheia/releases/latest/download/aletheia-x86_64-linux -o aletheia
...
export ALETHEIADB_EMBEDDING_MODEL=BAAI/bge-small-en-v1.5</pre>
            ...
            <div>
              ...
              <pre class="bg-black/40 rounded-lg p-4 text-sm text-tertiary overflow-x-auto">./aletheia
Engine listening on http://localhost:3000</pre>
            ...
            <div>
              ...
              <pre class="bg-black/40 rounded-lg p-4 text-sm text-tertiary overflow-x-auto">curl -X POST http://localhost:3000/ingest -H "Content-Type: application/json" -H "x-api-key: your-secret-key" -d 'body'</pre>
            </div>
          </div>
        </div>
        ...
        <div class="rounded-xl border border-outline-variant/10 bg-surface-container-low p-8">
          ...
          <p class="text-sm text-tertiary mb-4">
            For organizations requiring dedicated infrastructure with full data sovereignty.
            We provide Terraform modules for provisioning in your AWS/GCP/Azure account.
          </p>
