...
const plans = [
  ...
  },
  {
    ...
    price: "$12.00",
    unit: "/month",
    description: "Azure Standard_D2s_v5 dedicated VM",
    features: ["2 vCPUs | 8 GiB RAM", "Isolated SQLite substrate", "Best for sandboxing & dev", "Aletheia cut included"],
    cta: "Deploy VM",
    highlighted: false,
  },
  {
  ...
  },
  {
    ...
    price: "$90.00",
    unit: "/month",
    description: "Azure Standard_D4s_v5 dedicated VM",
          ...
          <div>
            ...
            <nav class="space-y-1">
              <button
                ...
                onClick$={() => { activeMissionTab.value = "settings"; }}
              >
                <SettingsIcon class={`w-4 h-4 ${activeMissionTab.value === "settings" ? "text-primary" : "text-tertiary"}`} />
                <span>Settings</span>
              </button>
            </nav>
          </div>
        ...
        <header class="mb-8 flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
          {activeMissionTab.value === "billing" ? (
            ...
            <div>
              ...
              <p class="mt-1.5 text-sm text-tertiary">Manage your account, team, and memory context templates.</p>
            </div>
          ) : (
            <div>
          ...
          {activeMissionTab.value !== "billing" && activeMissionTab.value !== "settings" && (
        ...
        {activeMissionTab.value === "settings" && (
          <div class="mb-8 inline-flex rounded-xl border border-outline-variant/10 bg-surface-container-low p-1">
            <button
              ...
              onClick$={() => { activeSettingsTab.value = "profile"; }}
            >
              <UserIcon class={`w-3.5 h-3.5 ${activeSettingsTab.value === "profile" ? "text-on-primary" : "text-tertiary"}`} />
              Profile
            </button>
            <button
              type="button"
              ...
              onClick$={() => { activeSettingsTab.value = "team"; }}
            >
              <UsersIcon class={`w-3.5 h-3.5 ${activeSettingsTab.value === "team" ? "text-on-primary" : "text-tertiary"}`} />
              Team
            </button>
            <button
              type="button"
              ...
              onClick$={() => { activeSettingsTab.value = "templates"; }}
            >
              <FileTextIcon class={`w-3.5 h-3.5 ${activeSettingsTab.value === "templates" ? "text-on-primary" : "text-tertiary"}`} />
              Context Templates
            </button>
          </div>
        )}
              ...
              <div class="max-w-2xl space-y-6">
                <div class="rounded-xl border border-outline-variant/10 bg-surface-container-low p-6">
                  <div class="flex items-start gap-5">
                    <div class="h-16 w-16 shrink-0 rounded-full bg-gradient-to-br from-primary to-secondary flex items-center justify-center text-xl font-bold text-on-primary shadow-sm">
                      {platformData.value.user.username.slice(0, 2).toUpperCase()}
                    </div>
                    <div class="pt-1">
                      <h2 class="text-lg font-bold text-on-surface">{platformData.value.user.username}</h2>
                ...
                <div class="rounded-xl border border-outline-variant/10 bg-surface-container-low divide-y divide-outline-variant/5">
                  <div class="p-5">
                    ...
                    <p class="text-sm font-semibold text-on-surface">{platformData.value.user.username}</p>
                  </div>
                  <div class="p-5">
                    <p class="text-[10px] font-bold uppercase tracking-[0.15em] text-tertiary mb-1.5">User ID</p>
                    <div class="flex items-center gap-2">
                      <code class="flex-1 text-xs font-mono text-tertiary bg-black/30 rounded-md px-3 py-2 border border-outline-variant/5 truncate">{platformData.value.user.user_id}</code>
                      <button
                        ...
                        onClick$={async () => {
                          await navigator.clipboard.writeText(platformData.value.user.user_id);
                          settingsCopiedId.value = "user_id";
                          setTimeout(() => settingsCopiedId.value = "", 2000);
                        }}
                        title="Copy User ID"
                      >
                        {settingsCopiedId.value === "user_id" ? <CheckIcon class="w-3.5 h-3.5 text-green-400" /> : <CopyIcon class="w-3.5 h-3.5" />}
                      </button>
                    </div>
                  </div>
                </div>
            ...
            {activeSettingsTab.value === "team" && (
              <div class="max-w-2xl space-y-6">
                <div class="rounded-xl border border-outline-variant/10 bg-surface-container-low p-5">
                  <div class="flex items-center justify-between mb-4">
                    <div class="flex items-center gap-2">
                      ...
                      <h2 class="text-sm font-bold text-on-surface">Members ({platformData.value.members.length})</h2>
                    </div>
                  </div>
                  <div class="space-y-1">
                    {platformData.value.members.map((m: TeamMemberInfo, i: number) => (
                ...
                <div class="rounded-xl border border-outline-variant/10 bg-surface-container-low p-5">
                  ...
                  <form method="post" action="/api/team" class="flex flex-col gap-3 sm:flex-row">
                    <input
                      type="email"
                      name="email"
            ...
            {activeSettingsTab.value === "templates" && (
              <div class="max-w-3xl space-y-6">
                      ...
                      <div key={p.name} class="group relative rounded-xl border border-outline-variant/10 bg-surface-container-low p-4 transition-all hover:border-primary/20 hover:shadow-sm">
                        <div class="flex items-center justify-between mb-2">
                          ...
                          <button
                            ...
                            onClick$={() => { settingsNewName.value = p.name; settingsNewTemplate.value = p.template; showSettingsCreateForm.value = true; }}
                          >
                            Use
                          </button>
                        </div>
                ...
                <div class="flex items-center justify-between">
                  <h3 class="text-[10px] font-bold uppercase tracking-[0.15em] text-tertiary">Your Templates ({platformData.value.templates.length})</h3>
                  <button
                    onClick$={() => { showSettingsCreateForm.value = true; settingsNewName.value = ""; settingsNewTemplate.value = ""; }}
                ...
                {showSettingsCreateForm.value && (
                  <Form action={createTemplateAction} class="rounded-xl border border-primary/20 bg-surface-container-low p-5 space-y-4">
                    <h3 class="text-sm font-bold text-on-surface">Create Template</h3>
                    <input type="hidden" name="cluster_id" value={platformData.value.clusterId} />
                    <div>
                      ...
                      <input
                        ...
                        onInput$={(_, el) => { settingsNewName.value = el.value; }}
                      />
                    </div>
                    <div>
                      ...
                      <textarea
                        ...
                        onInput$={(_, el) => { settingsNewTemplate.value = el.value; }}
                      />
                    </div>
                    <div class="flex gap-3">
                      <button type="submit" disabled={createTemplateAction.isRunning} class="rounded-lg bg-primary px-5 py-2.5 text-xs font-bold text-on-primary hover:opacity-90 disabled:opacity-50 transition-opacity shadow-sm">
                        {createTemplateAction.isRunning ? "Saving..." : "Save Template"}
                      </button>
                      <button type="button" onClick$={() => showSettingsCreateForm.value = false} class="rounded-lg border border-outline-variant/15 px-5 py-2.5 text-xs font-bold text-tertiary hover:bg-surface-container transition-colors">
                        Cancel
                      </button>
                    </div>
                  </Form>
                ...
                {/* Template List */}
                {platformData.value.templates.length === 0 && !showSettingsCreateForm.value && (
