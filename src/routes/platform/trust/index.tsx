...
import { ShieldCheckIcon, LockIcon, FileTextIcon, ServerIcon, EyeIcon } from "lucide-qwik";
      ...
      <main class="flex-1 overflow-y-auto p-8 lg:p-12 mb-20 max-w-5xl mx-auto w-full pt-[104px]">
        <header class="mb-16 text-center">
          <div class="inline-flex items-center gap-2 rounded-full border border-green-500/30 bg-green-500/10 px-4 py-1.5 mb-6">
            ...
            <span class="font-mono text-[11px] font-bold uppercase tracking-[0.2em] text-green-400">Enterprise Grade</span>
          </div>
        ...
        <div class="space-y-8">
          {sections.map(({ title, icon: Icon, items }) => (
            <div key={title} class="rounded-2xl border border-outline-variant/10 bg-surface-container-low p-8">
              ...
              <ul class="space-y-3">
                {items.map((item) => (
                  <li key={item} class="flex items-start gap-2 text-sm text-tertiary">
        ...
        <div class="mt-12 rounded-2xl border border-primary/20 bg-primary/5 p-8 text-center">
          ...
          <p class="text-tertiary mb-6">We provide security questionnaires, penetration test summaries, and architecture diagrams for enterprise procurement.</p>
          <a href="mailto:trust@aletheiadb.com" class="inline-block rounded-xl bg-primary px-8 py-3 font-bold text-white hover:opacity-90 transition-opacity">
