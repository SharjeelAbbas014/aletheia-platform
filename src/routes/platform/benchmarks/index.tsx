      ...
      <main class="flex-1 overflow-y-auto p-6 lg:p-12 mb-20 max-w-6xl mx-auto w-full pt-[104px] relative z-10">
        <header class="mb-14 text-center">
          ...
          <p class="text-muted text-base lg:text-lg max-w-2xl mx-auto leading-relaxed">
            Transparent, reproducible evaluation of AletheiaDB against industry leaders on standard agent memory benchmarks.
          </p>
        </header>
                  ...
                  return (
                    ...
                    >
                      <td class="py-4 font-headline text-sm font-bold text-white">
                        {isAletheia ? <span class="text-primary flex items-center gap-1.5">★ {ds.name}</span> : ds.name}
                      </td>
                      {ds.values.map((v, i) => (
                        <td key={i} class="py-4 px-3 text-right font-mono text-sm">
                          <span class={isAletheia ? "text-primary font-bold drop-shadow-[0_0_8px_rgba(192,193,255,0.4)]" : "text-muted"}>
                            {v.toFixed(1)}%
                          </span>
                        </td>
                      ))}
            ...
            {datasets.map((ds) => {
              ...
              return (
                <div key={ds.name} class="flex flex-col md:flex-row md:items-center gap-2 md:gap-4">
                  <span class={`w-28 text-sm font-headline font-bold ${isAletheia ? "text-primary" : "text-white"}`}>
                    {ds.name}
                  </span>
                  <div class="flex-1 h-9 rounded-xl bg-black/40 border border-outline-variant/10 p-0.5 overflow-hidden flex items-center">
                    ...
                    >
                      <span class={isAletheia ? "text-white drop-shadow-md" : "text-white drop-shadow-sm"}>
            ...
            <div class="flex gap-4 p-4 rounded-xl bg-surface-container-low/30 border border-outline-variant/10">
              ...
              <div>
                ...
                <p class="text-xs text-muted leading-relaxed">
                  ...
                  <a href="https://github.com/sharjeel619/aletheia" target="_blank" rel="noreferrer" class="text-primary hover:underline ml-1">
                    github.com/sharjeel619/aletheia <ExternalLinkIcon class="w-3 h-3 inline mb-0.5" />
                  </a>
                </p>
              </div>
            </div>
