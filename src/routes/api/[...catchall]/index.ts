...
export const onRequest: RequestHandler = async (event) => {
  ...
  try {
    ...
    if (isFractional) {
      ...
      if (tokenBalance <= 0) {
        captureError(new Error("Token balance exhausted"), { action: "apiProxy", context: "tokenExhausted", userId });
    ...
    if (clusterTier !== "self_hosted") {
      ...
      if (!rateResult.allowed) {
        captureError(new Error("Rate limit exceeded"), { action: "apiProxy", context: "rateLimited", userId, tier: clusterTier });
    ...
    try {
      ...
      captureError(err, { action: "apiProxy", context: "engineProxy", proxyUrl });
    ...
    captureError(e, { action: "apiProxy", method: request.method, url: request.url });
